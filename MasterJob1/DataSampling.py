...
def datasampling(num=1, length=10, **kwargs):
    ...
    for i in range(num):
        ...
        if data_type == int:
            ...
            result.append(
                [random.randint(kwargs.get('range_start', -100), kwargs.get('range_end', 100)) for i in range(length)])
        elif data_type == tuple:
            result.append(tuple(
                random.randint(kwargs.get('range_start', -100), kwargs.get('range_end', 100)) for i in range(length)))
        elif data_type == dict:
            result.append({i: random.randint(kwargs.get('range_start', -100), kwargs.get('range_end', 100)) for i in
                           range(length)})
        else:
            raise ValueError('Unsupported data type')
    result.append(random.randint(0, 1))   # 随机增加真实标签
    return result
